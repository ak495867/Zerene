...
class ParticipantRiskState:
    ...
    """
    def __init__(self, owner_id: str, initial_capital: float = 1_000_000.0, limits: Optional[RiskLimits] = None):
        self.owner_id = owner_id
        self.initial_capital = initial_capital
        self.peak_equity = initial_capital
        self.realized_pnl = 0.0
        self._dirty: bool = False
        self.positions: PositionDict = PositionDict(self)       # symbol -> net quantity (positive=long, negative=short)
        self.average_prices: PositionDict = PositionDict(self)  # symbol -> avg entry price
    ...
    def calculate_var_cvar(self, confidence: float = 0.95) -> Tuple[float, float]:
        ...
        var_return = abs(sorted_returns[cutoff_idx]) if sorted_returns[cutoff_idx] < 0 else 0.0
...
class RiskEngine:
    ...
    def register_participant(self, owner_id: str, initial_capital: float = 1_000_000.0, limits: Optional[RiskLimits] = None) -> ParticipantRiskState:
        if owner_id not in self.states:
            self.states[owner_id] = ParticipantRiskState(owner_id, initial_capital, limits)
    ...
    def validate_order(self, order: Order) -> Tuple[bool, Optional[str]]:
        ...
        if price <= 0:
            return True, None  # Market order without reference price passes check until matched

        # Reduce-only check
        if order.order_type == OrderType.REDUCE_ONLY:
            current_pos = state.positions.get(order.symbol, 0.0)
            if (order.side == Side.BUY and current_pos >= 0) or (order.side == Side.SELL and current_pos <= 0):
        ...
        sym_limit = state.limits.symbol_position_limits.get(order.symbol, state.limits.max_position_per_symbol)
        current_pos = state.positions.get(order.symbol, 0.0)
    ...
    def on_trade_fill(self, trade: Trade) -> None:
        ...
        for owner_id, side in [(trade.maker_owner_id, Side.SELL if trade.aggressor_side == Side.BUY else Side.BUY),
            ...
            if abs(current_qty) > 1e-9:
                state.running_gross_exposure = max(0.0, state.running_gross_exposure - abs(current_qty) * trade.price)
                state.running_net_exposure -= current_qty * trade.price
                if avg_price > 0:
                    state.cached_unrealized_pnl -= current_qty * (trade.price - avg_price)
            ...
            if (current_qty > 0 and trade_delta < 0) or (current_qty < 0 and trade_delta > 0):
            ...
            elif (current_qty >= 0 and trade_delta > 0) or (current_qty <= 0 and trade_delta < 0):
                new_avg = ((abs(current_qty) * avg_price) + (abs(trade_delta) * trade.price)) / abs(new_qty)
            ...
            state.update_peak_equity(unreal)
            dd = state.calculate_drawdown(unreal)
            if dd >= state.limits.max_drawdown_pct or (-state.realized_pnl >= state.limits.max_daily_loss):
                state.kill_switch_active = True
