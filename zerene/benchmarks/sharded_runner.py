...
class ShardedBenchmarkRunner:
    ...
    def run(
        ...
        # Aggregate results
        total_ops = sum(
            r["workload_metadata"]["total_operations"] for r in completed_results
        )
        total_trades = sum(
            r["execution_metrics"]["total_trades_generated"] for r in completed_results
        )
        total_cancels = sum(
            r["execution_metrics"]["successful_cancels"] for r in completed_results
        )
        total_modifies = sum(
            r["execution_metrics"]["successful_modifies"] for r in completed_results
        )
        ...
        avg_p50 = (
            sum(r["latencies_ns"]["p50_ns"] for r in completed_results)
            / self.num_shards
        )
        avg_p90 = (
            sum(r["latencies_ns"]["p90_ns"] for r in completed_results)
            / self.num_shards
        )
        avg_p99 = (
            sum(r["latencies_ns"]["p99_ns"] for r in completed_results)
            / self.num_shards
        )
        avg_p99_9 = (
            sum(r["latencies_ns"]["p99_9_ns"] for r in completed_results)
            / self.num_shards
        )
        ...
        return {
            ...
            "workload_metadata": {
                ...
                "limit_inserts_pct": completed_results[0]["workload_metadata"][
                    "limit_inserts_pct"
                ],
                "market_inserts_pct": completed_results[0]["workload_metadata"][
                    "market_inserts_pct"
                ],
                "cancels_pct": completed_results[0]["workload_metadata"]["cancels_pct"],
                "modifies_pct": completed_results[0]["workload_metadata"][
                    "modifies_pct"
                ],
                "latency_sample_size": sum(
                    r["workload_metadata"]["latency_sample_size"]
            ...
            "execution_metrics": {
                ...
                "avg_matches_per_order": round(
                    total_trades / max(1, (total_ops * 0.7)), 2
                ),  # Approx
                "successful_cancels": total_cancels,
                "successful_modifies": total_modifies,
                "final_orderbook_depth_levels": sum(
                    r["execution_metrics"]["final_orderbook_depth_levels"]
                    for r in completed_results
                ),
            },
            "latencies_ns": {
