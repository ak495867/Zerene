...
class ExecutionAlgorithm(ABC):
    ...
    def _acquire_slice_order(
        ...
        if self.use_passive_pegging and urgency < self.urgency_threshold:
            ...
            if self.side == Side.BUY and best_bid is not None:
                ...
                price = (
                    best_bid
                    if best_ask is None or best_ask - best_bid > 0.02
                    else round(best_bid + 0.01, 2)
                )
            elif self.side == Side.SELL and best_ask is not None:
                order_type = OrderType.LIMIT
                price = (
                    best_ask
                    if best_bid is None or best_ask - best_bid > 0.02
                    else round(best_ask - 0.01, 2)
                )
    ...
    @abstractmethod
    def get_next_slice(
        self, current_time: float, market_data: Dict[str, Any]
    ) -> Optional[Order]:
...
class TWAPExecution(ExecutionAlgorithm):
    ...
    def get_next_slice(
        ...
        urgency = min(
            1.0,
            (1.0 - (time_left / total_time))
            * ((self.total_quantity - self.executed_quantity) / self.total_quantity),
        )

        return self._acquire_slice_order(
            "TWAP", qty, current_time, market_data, urgency
        )
...
class VWAPExecution(ExecutionAlgorithm):
    ...
    def get_next_slice(
        ...
        if (
            self.is_finished
            or self.current_slice_idx >= self.slices
            or current_time < self.next_trigger_time
        ):
            return None

        weight = self.weights[self.current_slice_idx]
        qty = min(
            self.total_quantity * weight, self.total_quantity - self.executed_quantity
        )
        ...
        return self._acquire_slice_order(
            "VWAP", qty, current_time, market_data, urgency
        )
...
class POVExecution(ExecutionAlgorithm):
    ...
    def get_next_slice(
        ...
        self.executed_quantity += qty
        return self._acquire_slice_order(
            "POV", qty, current_time, market_data, urgency=0.1
        )
...
class ImplementationShortfallExecution(ExecutionAlgorithm):
    ...
    def __init__(
        ...
        for j in range(T):
            w = (
                math.sinh(self.kappa * (T - j)) - math.sinh(self.kappa * (T - j - 1))
            ) / math.sinh(self.kappa * T)
