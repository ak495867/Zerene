...
class MarketImpactModel(ABC):
    ...
    @abstractmethod
    def calculate_permanent_impact(self, quantity: float, total_daily_volume: float) -> float:
...
class AlmgrenChrissImpact(MarketImpactModel):
    ...
    def calculate_permanent_impact(self, quantity: float, total_daily_volume: float = 1e6) -> float:
