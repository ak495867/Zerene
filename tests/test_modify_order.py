...
def test_priority_preservation_on_quantity_reduction():
    ...
    is_ok, modified, reason = book.modify_order(
        "B1", new_quantity=6.0, new_price=None, timestamp=3.0
    )
...
def test_priority_loss_on_quantity_increase_or_price_change():
    ...
    is_ok, mod, reason = book.modify_order(
        "B1", new_quantity=15.0, new_price=100.0, timestamp=4.0
    )
...
def test_matching_engine_and_venue_modify_crossing():
    ...
    is_ok, mod_order, reason = venue.modify_order(
        "ETH-USD", "B1", new_quantity=5.0, new_price=2000.0
    )
    assert is_ok is True
