...
def test_sip_feed_simulator():
    ...
    snap_fast = OrderBookSnapshot("BTC-USD", 1.0, [(101.0, 10.0)], [(101.5, 10.0)], 101.25, 0.5, 0.0)
    snap_slow = OrderBookSnapshot("BTC-USD", 1.0, [(100.0, 10.0)], [(100.5, 10.0)], 100.25, 0.5, 0.0)
    ...
    arbs = sip.calculate_latency_arbitrage_opportunity({"VENUE_B": snap_fast}, {"VENUE_B": snap_slow})
    assert len(arbs) > 0
...
def test_footprint_tracker():
    ...
    t1 = Trade("T1", "M1", "TK1", "BTC-USD", price=100.0, quantity=5.0, aggressor_side=Side.BUY, timestamp=1.0)
    t2 = Trade("T2", "M2", "TK2", "BTC-USD", price=100.0, quantity=2.0, aggressor_side=Side.SELL, timestamp=2.0)
