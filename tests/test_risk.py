...
def test_kill_switch_on_drawdown():
    ...
    state = risk.register_participant(
        "TRADER_DD", initial_capital=100_000.0, limits=limits
    )
    ...
    trade = Trade(
        ...
        timestamp=1.0,
        maker_owner_id="TRADER_DD",
        taker_owner_id="OTHER",
    )
...
def test_kill_switch_mass_cancel_resting_orders():
    ...
    limits = RiskLimits(max_drawdown_pct=0.10)
    state = venue.risk_engine.register_participant(
        "TRADER_BLOWUP", initial_capital=100_000.0, limits=limits
    )

    # Place resting limit and stop orders on venue
    o_limit = Order(
        order_id="REST-1",
        client_order_id="C-R1",
        symbol="BTC-USD",
        side=Side.BUY,
        order_type=OrderType.LIMIT,
        price=45000.0,
        quantity=1.0,
        owner_id="TRADER_BLOWUP",
    )
    o_stop = Order(
        order_id="STOP-1",
    ...
    assert (
        o_limit.reject_reason is not None
        and "RISK_LIMIT_BREACH" in o_limit.reject_reason
    )
