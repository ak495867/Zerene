...
def test_call_auction_uncrossing():
    ...
    # Bids
    engine.add_order(
        Order("B1", "CB1", "BTC-USD", Side.BUY, OrderType.LIMIT, 10.0, price=105.0)
    )
    engine.add_order(
        Order("B2", "CB2", "BTC-USD", Side.BUY, OrderType.LIMIT, 5.0, price=102.0)
    )

    # Asks
    engine.add_order(
        Order("A1", "CA1", "BTC-USD", Side.SELL, OrderType.LIMIT, 8.0, price=100.0)
    )
    engine.add_order(
        Order("A2", "CA2", "BTC-USD", Side.SELL, OrderType.LIMIT, 10.0, price=104.0)
    )
...
def test_self_trade_prevention_cancel_newest():
    ...
    sell_order, trades = engine.process_order(
        Order(
            ...
            price=100.0,
            owner_id="TRADER_A",
            stp_mode=STPMode.CANCEL_NEWEST,
        )
    )
...
def test_self_trade_prevention_cancel_oldest():
    ...
    sell_order, trades = engine.process_order(
        Order(
            ...
            price=100.0,
            owner_id="TRADER_A",
            stp_mode=STPMode.CANCEL_OLDEST,
        )
    )
...
def test_marl_environment():
    env = MultiAgentRLMarketEnv(
        agent_ids=["agent_0", "agent_1"], symbol="BTC-USD", max_steps=10
    )
    obs = env.reset()
...
def test_span_portfolio_risk_and_liquidation():
    ...
    liq_orders = span.execute_liquidation_cascade(
        "FUND_X", exchange, market_prices_crash
    )
